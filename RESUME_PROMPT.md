...
last-updated: 2026-05-02 (after Phase 16 — `harness init` adoption wizard)
...
**Status:** Implementation in progress. Phase 16 init landed; Phases 0–14 + 14.x + 16 in tree (~20 founder-days). Phase 15 (trial-run pilot) is next — operator-driven, exercises the just-shipped init against mypal end-to-end. **Documentation in `docs/` is still the source of truth for design; the code in `harness/` is the runtime that implements it.**
...
### Twenty-two sensors green (smoke:init added — pure mechanical, no claude burn)
...
- ~~**No init script.**~~ **Landed in Phase 16.** `harness init` is now the adoption wizard. Profile-FREE per operator decision 2026-05-02: detection-driven, runs against any project layout. Two operator dialogs (proceed? + E2E now/defer/skip), under L44 cap. Inquirer was specced but rejected — hand-rolled readline keeps deps lean. The "now" branch prints the three setup commands but does NOT exec them yet — automation lands in Phase 16.x.
...
- `now` → prints `setup:uat-browsers` + `setup:uat-sql --install [--driver sqlite|postgres|mysql]` + `setup:uat-docker` for the operator to run; `e2e_setup: now` recorded in `.harness/config.yaml`
