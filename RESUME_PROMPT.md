...
last-updated: 2026-05-03 (after Phase 16.2/16.3 — Discord QOL bundle + agent ask-operator MCP tool)
...
**Status:** Implementation in progress. Phase 16.3 (agent ask-operator MCP tool) landed; Phases 0–14 + 14.x + 16 + 16.1/16.2/16.3 in tree (~21 founder-days). Phase 15 (trial-run pilot) is next — operator-driven, exercises the now-fully-guided init + Discord polish + agent question-asking against mypal end-to-end. **Documentation in `docs/` is still the source of truth for design; the code in `harness/` is the runtime that implements it.**
...
|-------------|-------|------|
| `a66bfaf` | 16.3 | **`harness_ask_operator` MCP tool — agent pauses + asks mid-run.** New `harness/src/mcp/tools/ask-operator.ts` registers an 18th MCP tool. Agent calls `harness_ask_operator(run_id, question, options[]?, category?, timeout_ms?)` whenever it hits ambiguity / needs permission for a non-recoverable action / is stuck / wants to verify its interpretation. Tool writes `.harness/runs/active/<run_id>/questions/<id>.q.json`, polls every 500ms for `<id>.a.json`, returns `{answer, choice_id?, free_text?, timed_out?}` to the agent. Orchestrator gains `questionsWatcher` chokidar on `.harness/runs/active/` (depth 3) — `absorbQuestionFile` reads the question, looks up `meta.json` → channelId, fires `adapter.requestDialog` with `pingOperators: true` (Discord renders `<@id>` mentions for `DISCORD_OWNER_USER_IDS` so the operator gets a mobile push), 4-letter buttons (A/B/C/D) when options provided OR a single `👍 ack` button when free-form, `🛑` prefix + category badge (`[AMBIGUITY]`, `[PERMISSION]`, `[STUCK]`, `[VERIFY]`) so agent-initiated dialogs are visually distinct. On operator answer, writes `<id>.a.json` with `{answered_at, answer, choice_id?, free_text?, timed_out?}`. `DialogSpec` gains optional `pingOperators` field; Discord adapter prefixes content with `<@id>` mentions when set (mentions don't render inside embed descriptions, so when prompt > 1800 chars and we use embed body, ping prefix sits on `content`). **Bonus fix:** queue restore in `TaskQueue.load` now drops shadow entries whose `inbox_file` is gone, fixing the ENOENT-rename loop where abandoned-but-not-fully-cleaned tasks kept cycling on every orchestrator startup. Pre-flight `dispatch` channel-alive probe (16.2) + this drop together = stale queues no longer poison the run. |
| `f332544` | 16.2 | **Discord QOL bundle — embed-everywhere + Tier-0 activity feed.** Operator pivot: "channel only has nice formatted embeds by the end and the user is FULLY informed of what's happening during executing." Six successive fixes commit by commit: (a) `0b3e6b4` — colored embed live status edit-in-place, 👀/✅/❌/⚡ reactions on dialog clicks, embed-rendered approval bundles; (b) `7ab96a4` — short letter buttons (A/B/C/D) + full text in prompt body since Discord caps button labels at 80 chars; (c) `6e3f20c` — `customId` parser fix for multi-segment bundleIds (`TSK-XXX:Q1`); (d) `4efaec7` — `deadChannels: Set<string>` flagging on first 10003 stops `sendTyping` heartbeat spam; (e) `eb41191` — `adapter.isChannelAlive` preflight in `dispatch()` abandons stale-channel tasks before burning LLM quota; (f) `4e17fea` — `interaction.deferUpdate()` replaces `Recorded: x` ephemeral spam, prior dialog messages compact-edit on click (`_✅ Answered with **X**_` appended, buttons stripped), bodies > 2000 chars split on paragraph→line→space boundaries via `sendChunked`. Then this commit (`f332544`): task drop is now an embed (slate, body in description, pipeline footer); `postTaskUpdate.body` wraps in phase-colored embed via `buildContentEmbed` + `splitForEmbedDescription` (4096-char cap, multi-page footer); `PostUpdate` gains optional `activity` field; `buildPhaseEmbed` renders it as a 1024-char field. **NEW MODULE `harness/src/orchestrator/activity-summarizer.ts`** — Tier-0 (Ollama llama3.2:3b) summarizer of stream-jsonl events. `summarizeActivity({events})` takes a sliding window of last 30 events, extracts tool_use names + paths/commands + agent text snippets via `compactEvents`, prompts llama3.2:3b with anti-fluff system: "ONE LINE, max 120 chars, present-progressive ('Reading X', 'Editing Y', 'Running tsc'). No emoji, no markdown, no quotes. Falls back to 'Working…'." Orchestrator's running phase calls `startActivityFeed(entry, meta, window)` — onEvent appends to a 60-item sliding window, first summary fires 5s in, subsequent every 8s; updates the live status embed via `postTaskUpdate({activity})` only when summary changes. Silent fail when Ollama unreachable. Operator sees the agent's tool calls in plain English instead of a static yellow "running" badge for 60-300s. |
| `9d842fd` | 16.1 | Init becomes FULL guided setup per operator pushback "this should be a FULL guided setup." Per-missing-prerequisite dialogs replace bare warnings: claude (instruct-only), whisper (curl download via `downloadWhisperModel`), ollama (`brew install` + `ollama pull` via `offerInstallOllama` + `pullOllamaModel`), discord token + guild (inquirer `password()` prompt + `freeTextWithDefault` for guild → write to `~/.local/harness/.env` mode 0600 via `upsertHarnessEnv`). E2E `now` branch ACTUALLY EXECUTES `setup:uat-browsers` + `setup:uat-sql --build-binding` + `setup:uat-docker` via `runHarnessSetupScript` (spawns `pnpm exec tsx scripts/setup-X.ts` from harness pkg root w/ stdio inherited). Pre-configured environment skips ALL guided dialogs — for already-set-up operators, init still runs proceed + e2e and that's it. New `harness/src/init/secrets.ts` (harnessEnvPath = `~/.local/harness/.env`; `readHarnessEnv` parses dotenv; `upsertHarnessEnv` merges updates preserving comments + key order, mode 0600 on write). New `harness/src/init/setup-runners.ts` (downloadWhisperModel, pullOllamaModel, offerInstallOllama, runHarnessSetupScript — each spawns w/ inherited stdio so operator sees curl/brew/pnpm progress live). Inquirer `password` + `confirm` added to prompts.ts (operator preference per 2026-05-02 over hand-rolled). run.ts `loadEnvFiles` now prefers `~/.local/harness/.env` first so the dlxed `harness run` picks up creds written by init. Smoke unchanged (auto mode bypasses runGuidedSetup wrapper). |
| `3c52e37` | 16 | `harness init` adoption wizard — profile-FREE per operator decision 2026-05-02 ("should be agnostic, why are we shipping with profiles"). New module `harness/src/init/` (detect.ts: detectStackSignatures via marker files [package.json→typescript, pyproject.toml/requirements.txt→python, Gemfile→ruby, go.mod→go, Cargo.toml→rust, mix.exs→elixir, fallback→unknown — multiple sigs OK so polyglot repos enumerate cleanly]; detectAvailableSensors per signature [tsc + eslint for TS w/ tsconfig + .eslintrc*; ruff + mypy for Python via pyproject.toml [tool.X] sections; rubocop + brakeman for Ruby Gemfile dep regex; go vet + gofmt; cargo check + cargo clippy; mix compile --warnings-as-errors]; detectStartCommand best-guess [package.json scripts.dev > scripts.start for TS; manage.py runserver / Procfile web: line for Python; bin/rails server for Ruby; go run ./... fallback; cargo run]; detectHookCapability [.claude/ → claude-code; .git/ → git-hooks; else cli-only]; detectEnvironment advisory: claudeIsAvailable + ~/.local/harness/models/ggml-large-v3-turbo-q5_0.bin + GET $OLLAMA_HOST/api/tags + DISCORD_BOT_TOKEN/DISCORD_GUILD_ID env presence — never blocking; detectAll composes everything into single DetectionResult. prompts.ts: hand-rolled readline-based squareIntoSquareHole<T>(prompt, choices, auto) supports interactive + auto modes — interactive prints `[a*] label` w/ `*` on default, enter on blank picks default, unknown choice falls to default w/ a notice; freeTextWithDefault for slug overrides; no inquirer dep per operator's "fewer moving pieces". seed.ts: walks harness/templates/ recursively via readdirSync+statSync, applyPlaceholders surgically rewrites only `.harness/config/{workflow.md,sensors.yaml}` substituting `<project_name>:` → `<slug>:` + `\`<project_name>\`` → `\`<slug>\``; collide-fail without --force. init.ts orchestrator: detectAll → printSummary + printAdvisoryWarnings → ONE proceed dialog (a yes / b cancel) → seed → write `.harness/config.yaml` overlay (slug + origin_url + stack_signatures + hook_capability + start_command + detected_sensor_commands + off_limits defaults + project_globs scaffold) → ensureMirror unless --skip-mirror or no origin → ONE E2E dialog (now/defer/skip) — under L44's 2-question cap; on `now` prints the three setup commands but does NOT exec yet (P16.x); records e2e_setup back into .harness/config.yaml idempotently; prints `cd <repo> && pnpm dlx @devplusllc/harness watch ... && ... run --frontend discord|stub` next-steps, surfacing missing DISCORD_BOT_TOKEN/GUILD as a stub fallback. CLI wired in src/cli/init.ts + src/cli/index.ts (`harness init [target-dir] [--slug NAME] [--force] [--skip-mirror] [--no-prompt] [--auto-e2e now\|defer\|skip]`). slugOverride normalized via existing `normalizeProjectName` (lowercase + non-alnum→underscore) so override is filesystem-safe. New `smoke:init` (6 steps — TS+ESLint repo flags both sensors + slug=demo_app + start_command=pnpm run dev; empty repo → stack=[unknown] + 0 sensors; runInit auto seeds 8 files incl. config.yaml; placeholder substitution lands `demo_app:` in workflow.md w/ NO unresolved `<project_name>:` left; re-run without --force preserves existing files + collisions reported + e2e_setup flips to `skip` on second run; --slug override normalized + propagated to workflow.md). PURE MECHANICAL — no claude burn. |
| `1486ed0` | 14.x | Refinement loop lifts loose `candidate_assertions:` → strict `assertions:`. New module `harness/src/decision-capture/refinement.ts` (proposeStrictAssertions Tier-1 default per workflow.md `decision_extractor: 1`, runDecisionRefinement orchestrator, validateLift re-validates each lift via DecisionAssertion zod incl. discriminated kind union — invalid → auto-demote w/ zod issues quoted in rationale; alignProposalsToCandidates fills gaps when proposer drops a candidate; renderDialogPrompt produces single 4-choice prompt under L44 cap w/ ✅/🟡/🟠/⚪ per-proposal status indicators + "Recs: lift=N, demote=N, skip=N. HIGH-only=N." footer). New `refinement-schema.ts` (JSON Schema for proposer output — kind enum mirrors DecisionAssertion union, strict_assertion is `additionalProperties: true` since runtime zod is source of truth). New `refinement-prompt.ts` (anti-fabrication: "prefer demote/skip over fabricated regex", lists all 11 kinds w/ required fields, defines lift/demote/skip semantics, `human_review_hint` always lifts cleanly). types.ts adds `RefinementProposal` (lift|demote|skip + confidence + strict_assertion? + rationale), `RefinementResult` (operator_choice 4-way enum + lifted/demoted/skipped counts + proposer_failed?), `RefinerInput`/`RefinerOutput`. writer.ts gains `liftCandidatesToAssertions` — reads accepted decision frontmatter via parseFrontmatter+parseYaml, builds new `assertions:` array (lift = `{id, kind, ...strict_assertion}`; demote = `{id, kind: human_review_hint, description: candidate_description}`), filters surviving candidates, rewrites file w/ stringifyYaml + body preserved, regenerates ledger, returns LiftResult. capture.ts chains `runDecisionRefinement` after `acceptDraft` — only when commit AND candidates exist AND `bypassRefinement !== true`; refinement throw is logged + swallowed (accept stands; candidates remain loose). New `bypassRefinement` + `refinementTier` + `refinementDialogTimeoutMs` + `refinementProposerOverride` on `RunDecisionCaptureArgs`; mirrored on `OrchestratorOptions` and forwarded by handleDirectionRow. New `smoke:decision-refinement` (7 steps — approve_all w/ mixed lift+auto-demote+hint resolves to 1 strict text_must_match + 2 human_review_hint; approve_high_only resolves HIGH lift strict + explicit demote→hint + LOW lift survives as candidate; skip leaves all 3 candidates intact; demote_all → 3 hint assertions; proposer throw → proposer_failed=true + NO dialog fired + candidates loose; e2e runDecisionCapture stub-extractor + stub-proposer + stub-adapter approve-all → 1 strict text_must_not_match live + ledger=1; LIVE haiku proposer on `query_must_filter_by` — produces zod-valid `{orm: drizzle, table: integration_oauth_tokens, columns: [user_id], operator: eq, require_combination: and, in_globs: [src/integrations/**/*.ts]}` first try). ~1 cheap haiku call (~$0.03). |
| `f482685` | 14 | Decision capture flow: `harness/src/decision-capture/` (types — DecisionExtractorInput w/ raw_text + author_id + received_at + source + accepted_decisions context + tier; DecisionExtractorOutput w/ subject + summary + scope_globs + supersedes? + candidate_assertions[] (kind+description+parameters loose) + confidence_signal + not_a_decision; DecisionDraft w/ id + draft_path + canonical_path + raw_text; DraftConfirmDecision = commit\|edit\|reject; DecisionCaptureResult); schema — JSON Schema enforcing required fields + assertion kind enum + maxItems:3 candidate cap; prompt — anti-fabrication system: "be the decision-extractor, not the policy author; when in doubt, set not_a_decision=true; emit-only-JSON; supersedes only when input EXPLICITLY revokes prior decision"; id allocator — `allocateDecisionId` scans `.harness/ground/decisions/{,_inbox/}*.md` for highest existing DEC-NNNN incl. drafts AND `.rejected.md` tombstones, returns next monotonic id (per L13.2 — rejected ids burn, never recycled); extractor — `runDecisionExtractor` Tier 1 default (Haiku per workflow.md `decision_extractor: 1`), validates parsed output via isOutput guard; writer — `writeDecisionDraft` mints DEC-NNNN.draft.md frontmatter (status:draft, decided_at, decided_by, scope_globs, supersedes, capture_source, capture_confidence) + body w/ Summary/Original direction/Scope/Candidate assertions sections — candidate_assertions stored under FRONTMATTER `candidate_assertions:` field NOT strict `assertions:` (loose proposals; lifted to `assertions:` only after Phase 14.x refinement so DecisionFrontmatter zod doesn't reject the draft); `acceptDraft` flips status:draft→accepted, moves to canonical, `writeDecisionsLedger` regenerates → assertions become live (when refined), stamps `superseded_by` on referenced prior decision when set; `rejectDraft` writes `.rejected.md` tombstone (NOT delete) so DEC-id is monotonically burned; capture — `runDecisionCapture` orchestrates extract → draft → adapter.requestDialog (🟢 commit / 🟡 edit / 🔴 not-a-decision / E) Other) → accept/edit/reject branch; on extractor `not_a_decision=true` short-circuits without writing draft; on dialog timeout treats as edit so draft survives in `_inbox/` for operator return-trip. Orchestrator wired w/ `isDirectionRow`/`directionTextOf`/`directionAuthorOf`/`directionChannelOf` helpers; `absorbInboxFile` routes slash:command=direction OR free_text:intent=direction rows through `handleDirectionRow` inline (independent of task FIFO; never burns sensors/UAT quota); `OrchestratorOptions.decisionExtractorTier` + `decisionConfirmTimeoutMs` overrides. `smoke:decision-capture` (6 steps — id allocator empty→DEC-0001 + advances past accepted DEC-0003 + draft DEC-0005 → DEC-0006; writeDecisionDraft frontmatter validates incl. status:draft; acceptDraft flips status + ledger contains DEC-0006; LIVE haiku extracts FK-denorm direction → not_a_decision=false subject mentions FK/denorm; commit-path runDecisionCapture w/ stub-extractor + stub-adapter dialogResponse=a → accepted_path canonical + ledger_size=1; reject-path → tombstone leaves DEC-id burned + allocator advances). ~1 cheap haiku call (~$0.03). |
...
### Twenty-two sensors green (smoke:init added — pure mechanical, no claude burn). The 18-tool MCP smoke now exercises `harness_ask_operator`.
...
pnpm -F @devplusllc/harness smoke:decision-refinement # 7 steps — approve_all mixed-mode resolves 1 strict + 2 hint, candidates removed; approve_high_only → HIGH lift strict + explicit demote→hint + LOW lift survives candidate; skip leaves candidates intact; demote_all → 3 hint assertions; proposer throw → proposer_failed=true, NO dialog fired, candidates loose; end-to-end commit→refine via stubs lands strict + cleared candidates + ledger=1; LIVE haiku proposes zod-valid `query_must_filter_by` (drizzle/user_id/AND) on a concrete candidate. ~1 cheap haiku call (~$0.03). SKIPS Step 7 only without `claude`.
pnpm -F @devplusllc/harness smoke:init # 6 steps — TS+ESLint repo flags slug=demo_app + tsc+eslint sensors + start_command=pnpm run dev; empty repo → stack=[unknown], 0 sensors; runInit --no-prompt --skip-mirror seeds 8 files; workflow.md `<project_name>:` substituted with slug + config.yaml carries detected stack/sensors/e2e_setup:defer; re-run without --force keeps existing files + flips e2e_setup→skip; --slug override normalized + propagates to workflow.md. PURE MECHANICAL — no claude burn.
```
...
- **No GC cron schedule.** `harness gc sweep|run` exists as a CLI; nightly `/loop` or systemd-timer wiring lands as part of adopter setup (per operator note 2026-05-02 "harness needs to USE the detection, not have it for itself").
- ~~**No init script.**~~ **Landed in Phase 16/16.1.** `harness init` is the adoption wizard. Profile-FREE (operator decision 2026-05-02: agnostic detection beats baked profiles). Inquirer-driven (operator's stated preference; v1's hand-rolled readline was rejected). Full guided setup flips on per-missing-prerequisite dialogs (claude / whisper / ollama / discord token+guild) and the E2E `now` branch actually execs `setup:uat-browsers` + `setup:uat-sql --build-binding` + `setup:uat-docker` via `runHarnessSetupScript`. Pre-configured environment skips guided dialogs and runs proceed + E2E only.
...
- ~~**Decision-capture refinement step.** Phase 14 v1 stores candidate_assertions under a loose `candidate_assertions:` frontmatter field; lifting them to the strict `assertions:` form (which Layer-D sensors enforce) needs Phase 14.x — operator edit-loop that fills in the schema-required parameters per assertion kind.~~ **Landed in Phase 14.x.** Refinement fires post-accept on commit when candidates exist; single 4-choice dialog (approve_all / approve_high_only / demote_all / skip); writer lifts strict shapes into `assertions:` and demotes ambiguous ones to `human_review_hint`. Layer-D sensors now enforce the lifted assertions on the next run.

The `harness init`, `harness watch`, `harness mirror`, `harness mcp serve`, `harness run`, and `harness gc {sweep,run}` CLIs work today. End-to-end ingest → tightener (with operator-walked ambiguity resolution + ship-anyway override) → mirror → agent (with `harness_ask_operator` mid-run dialog support) → sensors (Layer A/B/D + decision-assertions) → reviewer subagent (Layer C, fresh context) → UAT pipeline (multi-probe http/cli/ui/sql-sqlite/integration routing, evidence-file gate) → adapter approval → backprop (Layer §V — invariant + regex sensor + chore(invariants) commit on mirror) runs cleanly. The Discord per-task channel renders embed-only with a single live status that edits in place, a Tier-0 `activity` field updated every 8s during the running phase, 👀/✅/❌/⚡ reactions on operator clicks, and `<@id>` mobile pings on agent-initiated questions. GC composes five passes (frontmatter freshness, generator drift, stub-catalog hits, doc-gardening, quality-grades) with an auto-merge classifier, batch canary, and rollback-on-canary-fail. Decision capture turns Discord `/direction` slash + free_text classified as `direction` into draft DEC-NNNN files, fires the 🟢/🟡/🔴 confirm dialog, and on commit regenerates the decisions ledger. Next missing pieces: trial-run pilot, the eventual git-push step gated on the recomputed evidence hash, and adopter-side GC cron scheduling.
...
### Phase 14.x design notes (binding)

- **Refinement fires post-accept on commit, not as a separate command.** Reasoning: solo-dev orphan problem — lazy refinement via a separate `/refine` slash leaves candidates rotting under `candidate_assertions:` forever, killing L26's value claim ("mechanical assertions, fail loud"). At-commit timing keeps context fresh + avoids a second operator entry point. Latency cost (~2-4s for the second Haiku call) is acceptable; /direction is not a hot path.
- **Refinement fires only on commit decision; never on edit / reject / not-a-decision.** Edit may rewrite candidates on the next pass; reject discards everything; not-a-decision short-circuits before the draft.
- **Refinement also short-circuits when candidate_assertions is empty.** No-op early-exit; no proposer call burned, no dialog shown.
- **Single dialog, four choices, under L44 cap.** approve_all (lift+demote per recs) / approve_high_only (HIGH lifts only; explicit demotes still demote; LOW lifts survive as candidates) / demote_all (everything → human_review_hint) / skip (leave all loose). Per-candidate approval dialogs would multiply by N candidates and shred operator attention; the proposer pre-filters and the operator approves the bundle.
- **DecisionAssertion zod is the source of truth.** validateLift re-validates each lift via the production zod *before* the dialog fires — a malformed shape gets auto-demoted with the zod issues quoted in the rationale, surfaced to the operator. The proposer's JSON Schema only enforces the outer envelope (kind enum, required fields); strict_assertion is `additionalProperties: true` because doing it twice is just token burn.
- **Demote always becomes `human_review_hint` (the kind), not the top-level frontmatter field.** A `human_review_hint` ASSERTION (with id/kind/description) lands in the `assertions:` array next to strict ones. Layer-D treats it as soft. The top-level `human_review_hint:` string field is left untouched — it's a different surface (decision-level note vs. assertion-level rule).
- **Proposer failure ≠ accept rollback.** Refinement is best-effort. A timeout / malformed JSON / subprocess crash on the proposer logs `refinement-proposer threw — leaving candidates loose` + returns `proposer_failed: true`; the prior `acceptDraft` is NOT rolled back. Candidates stay under `candidate_assertions:` for the next refine pass. Failure mode parity: the smoke verifies NO dialog fires when the proposer throws, so the operator isn't asked to approve nothing.
- **Candidate ids carry through.** Both extractor and refiner use `<DEC-id>-A<NN>` ids on candidates. The lift writer reuses the same id on the resulting strict assertion (or demoted hint), so audit trail across `candidate_assertions:` → `assertions:` survives.
- **Tier default = haiku.** Per workflow.md `decision_extractor: 1`. The refiner is small JSON in / small JSON out; haiku is sufficient. Sonnet override (`refinementTier`) for edge cases. Live smoke confirmed haiku produces zod-valid `query_must_filter_by` first try (`{orm: drizzle, table, columns, operator: eq, require_combination: and, in_globs}`) so the model tier ladder isn't an under-fit.
- **alignProposalsToCandidates fills proposer gaps.** When the proposer drops a candidate (returns N-1 proposals for N candidates), the runner inserts an auto-demote rather than silently dropping the candidate. The dialog still shows N rows so the operator sees the complete picture.
...
Phase 16 is complete. Next is **Phase 15 — trial-run pilot** (`docs/INTEGRATION_PLAN.md` §5 Phase 15; ~2 founder-days). The pilot exercises `harness init` against mypal end-to-end: adopt → submit a real backlog item → watch full lifecycle → assert each acceptance criterion in §2 of the plan.
...
- **Phase 16 init script** (~1.5 days) — `npx @devplusllc/harness init` w/ stack-profile detection + E2E setup branch.
- **Phase 12.x — GC cron schedule** *(deferred to Phase 16 / adopter setup; not a Harness-repo concern per operator decision 2026-05-02)*

### Phase 16.3 design notes — agent ask-operator (binding)

- **Agent CAN stop and ask at ANY time.** The implementer is no longer fire-and-forget. When it hits ambiguity / needs permission / is stuck / wants to verify, it calls `harness_ask_operator(run_id, question, options[]?, category?, timeout_ms?)`. Default timeout 10 minutes; operator can extend per call.
- **Filesystem mailbox is the IPC, not direct callbacks.** The MCP server runs as a subprocess of the implementer's `claude` CLI; it can't reach into the orchestrator's process. So the tool writes a `<id>.q.json` file under `.harness/runs/active/<run_id>/questions/`, polls every 500ms for `<id>.a.json`. The orchestrator's chokidar watcher sees the question, fires the dialog, writes the answer. This works equally well for any future MCP transport (HTTP, ws) since the contract is the file pair.
- **Pings are scoped to agent-initiated dialogs.** `DialogSpec.pingOperators: true` triggers `<@id>` mentions for every entry in `DISCORD_OWNER_USER_IDS`. Routine confirmations (per-Q walks, e2e setup) leave the flag false to avoid push-notification fatigue. The only path that flips it is `absorbQuestionFile` in the orchestrator.
- **Pings ride on `content`, not embed.** Discord embeds don't render mentions. When the prompt body fits in 2000 chars we put the `<@id>` prefix + prompt directly on `content`; when prompt exceeds 1800 we move the body to an embed and the ping prefix sits alone on `content` so push notifications still trigger.
- **Categories are advisory not gating.** `ambiguity` / `permission` / `stuck` / `verify` render as a `[CATEGORY]` badge in the prompt + are persisted on the question file. The dialog flow doesn't change per category; the badge gives the operator instant context for triage.
- **Free-form replies degrade to `👍 ack`.** When the agent calls without `options[]`, the dialog gets a single `👍 ack` button. The agent receives `{answer: "ack", choice_id: "ack"}`. v1 doesn't capture free-text replies; that lands when the Discord adapter's message handler can route a follow-up message back into the question's answer file.
- **Stale queue protection.** `TaskQueue.load` drops shadow entries whose `inbox_file` no longer exists. Combined with the `dispatch()` channel-alive preflight (16.2 `eb41191`), stale Discord channels + abandoned-but-not-cleaned-up tasks no longer poison subsequent runs.

### Phase 16.2 design notes — Discord QOL bundle (binding)

- **Embed-only per-task channel.** Operator pivot: "the channel only has nice formatted embeds by the end and the user is FULLY informed of what's happening during executing." Initial task drop, live status, content posts, dialogs, approval bundles all go through `EmbedBuilder`. Plain `content.send` is reserved for `<@id>` ping prefixes and the activity-summary edits where embed re-render is unnecessary.
- **Live status edits in place.** `Map<taskId, messageId>` in the Discord adapter tracks which message embodies the task's status. `postTaskUpdate` edits in place when the message exists, falls back to send + remember on first call or when the message went missing. Phase color shifts as the run advances (slate → blue → yellow → orange → purple → cyan → indigo → green/red).
- **Tier-0 activity feed (no Anthropic plan burn).** During the running phase, a sliding window of the last ~60 stream-jsonl events feeds `summarizeActivity` (Ollama llama3.2:3b). First call 5s in, every 8s after; only updates the embed when the summary differs from last. Anti-fluff prompt: "ONE LINE, max 120 chars, present-progressive ('Reading X', 'Editing Y'). No emoji, no markdown, no quotes. Falls back to 'Working…' on Ollama failure." Operator sees what the agent is actively doing rather than a static yellow badge for 60-300s.
- **Per-question walk replaces single-blob feedback.** When the tightener returns ready=false with ≥2 candidate_resolutions per ambiguity, the orchestrator fires N sequential A/B/C/D button dialogs (one per ambiguity), then a final 🟢/⚡/🔴 confirm. Each per-Q dialog is its own message with the question + full option text in body + single-letter buttons (Discord caps button labels at 80 chars; long candidate text stays in the body). Resolutions append to the tightener's proposal under `## Operator-resolved ambiguities` so the implementer agent works against the operator's settled answers, not the LLM's defaults.
- **Reactions ack operator clicks.** 👀 on dialog post (waiting on operator), ✅ after a button records the operator's choice, ❌ on cancel/reject/edit, ⚡ on `ship_anyway`/ask. Combined with prior dialog message edit (buttons stripped + `_✅ Answered with **X**_` appended), channel scrollback shows what was answered without ambiguity.
- **No more `Recorded: x.` ephemeral spam.** `interaction.deferUpdate()` silently acknowledges the button without sending an ephemeral. The reaction + the prior-message compact-edit are the operator-visible feedback.
- **Body chunking, not truncation.** `splitForEmbedDescription` cuts on paragraph → line → space boundaries with a 4000-char per-chunk budget. Multi-page footers (`1 / 3`) tag the chunks. Same primitive used for plain content via `splitForDiscord` against the 1990-char content cap.
- **Dead-channel suppression.** `deadChannels: Set<string>` flags any channel that returns Discord error 10003 once. `postTaskUpdate`, `sendTyping`, `requestDialog`, `requestApproval` all check first and silently no-op if dead. `requestDialog`/`requestApproval` return their respective timed-out sentinels so the orchestrator's existing failure-branch handles graceful run-down. `isChannelAlive` exposes the same check to the orchestrator's pre-dispatch preflight; tasks whose channel is gone are abandoned (no LLM burn) and moved to `processed/failed/` cleanly.
- **Typing indicator during long phases.** `startTyping(channelId)` returns a stop fn; the orchestrator wraps tightener / implementer / reviewer / UAT calls so Discord shows "Harness is typing…" continuously. The 8s heartbeat refresh (Discord's native indicator decays at 10s) self-stops on dead-channel detection.

### Phase 16 design notes (binding)

- **Profile-free per operator decision 2026-05-02.** L47 (stack profiles) replaced by detection. Stack signatures are an array (typescript+python coexist), not a single profile. Sensors propose per-stack via marker-file presence; novel stacks (Bun, Deno) work as `unknown` without harness pkg patch.
- **Two-dialog cap (L44).** Init has exactly two operator interactions: proceed? and E2E now/defer/skip. Per-sensor approval was rejected — too much ceremony for a wizard that should "just work." Detected commands land in `.harness/config.yaml` `detected_sensor_commands` for operator inspection later.
- **No inquirer.** Hand-rolled readline-based prompts. `squareIntoSquareHole<T>(prompt, choices, auto)` supports both interactive (TTY) and auto (smoke / `--no-prompt`) modes. Interactive prints `[a*] label` w/ `*` on default, enter on blank picks default, unknown choice → fall to default with notice. Operator preference: fewer moving pieces.
- **E2E `now` does NOT yet exec setup commands.** v1 prints the three commands; operator runs them in their shell. P16.x adds the actual orchestration. Reason: setup:uat-* runs `pnpm add` / `npx playwright install` / `docker compose --version` against the user's project — surface for failure modes (network, docker not installed, pnpm vs npm vs yarn) is large enough that it deserves its own phase rather than landing as init complexity.
- **`<project_name>` substitution is surgical.** Only `.harness/config/{workflow.md,sensors.yaml}` get the placeholder rewrite. Other files (templates/.archive/README.md, etc.) pass through unchanged. The Liquid `{{project_name}}` placeholders in agent prompt bodies are NOT touched at init — those resolve at run time.
- **Slug override is normalized.** `--slug demo-app` → `demo_app` via existing `normalizeProjectName` so the override produces a filesystem-safe path matching mirror checkout conventions. Operator can pass dashes; harness converts.
- **Collide-fail without --force.** If `.harness/config/workflow.md` exists, init keeps the existing file + records a collision warning. Re-running with `--force` overwrites. The smoke verifies both paths.
- **Mirror init is best-effort.** Missing origin → skip with warning. Existing mirror → ensureMirror is idempotent (re-adopt no-ops).
- **Detection is pure / read-only.** No writes during the detect phase. Decision summary printed BEFORE the proceed dialog; operator can cancel having seen the full picture.

### Phase 16.1 — full guided init E2E branches (binding)

Per operator pivot, the init script asks "Set up E2E now / Defer / Skip" and branches:
- `now` → spawns `setup:uat-browsers` + `setup:uat-sql --build-binding` + `setup:uat-docker` directly via `runHarnessSetupScript` (stdio inherited so operator sees curl/brew/pnpm progress); `e2e_setup: now` recorded in `.harness/config.yaml`
- `defer` → `e2e_setup: defer` in `.harness/config.yaml`; orchestrator prompts again on first UAT need
- `skip` → `e2e_setup: skip`; code-class UAT becomes review-only; high-stakes refuses dispatch
