...
last-updated: 2026-05-02 (after Phase 14.x — refinement lifts candidate_assertions → strict)
...
**Status:** Implementation in progress. Phase 14.x complete — Phases 0–14 + 14.x refinement loop all landed (~19 founder-days). Phase 15 (trial-run pilot) is next. **Documentation in `docs/` is still the source of truth for design; the code in `harness/` is the runtime that implements it.**
...
### Twenty-one sensors green (smoke:decision-refinement added — ~1 cheap haiku call; smoke:backprop + smoke:gc + smoke:sensors pure mechanical-or-cheap)
...
Phase 14.x is complete. Next is **Phase 15 — trial-run pilot** (`docs/INTEGRATION_PLAN.md` §5 Phase 15; ~2 founder-days).
